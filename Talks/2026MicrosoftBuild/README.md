# Microsoft Build 2026 - Building your own MCP server

This is a talk I gave at [Microsoft Build 2026](https://build.microsoft.com/en-US/sessions/LTG466?source=sessions). The talk was in English.

## Talk Title

### Building your own MCP server

## Abstract

The Model Context Protocol is all the rage and rightly so. As the USB-C for AI it is an incredibly powerful tool that acts as a bridge between real-world systems and LLMs. But how do you get started with MCP and more importantly: how do you build your own MCP server?

In this session we'll build an MCP server from scratch using the MCP C# SDK. We'll look at how to build tools, prompts and resources and how to leverage these with our own AI agent in Microsoft Foundry.

## Resources and Links

### References

These are the links to resources and references I mentioned in my talk:

- The Office
- [DunderMifflin](https://github.com/StuartvdLee/DunderMifflin/tree/azurefest2025)
- [Model Context Protocol](https://modelcontextprotocol.io/)
- [Model Context Protocol C# SDK](https://github.com/modelcontextprotocol/csharp-sdk)
- [Getting Started with MCP (Model Context Protocol)](https://www.youtube.com/watch?v=DpyjAKmNwpI)
- [Connect to Model Context Protocol servers](https://learn.microsoft.com/en-us/azure/ai-foundry/agents/how-to/tools/model-context-protocol)
- [Slide deck](2026MicrosoftBuild.pdf)

Thank you for listening to my talk and checking out the resources!
